...
/* collapse order-level table to unique-customer level, and compute a few more complex metrics now
to be re-used in the next cte, for the sake of clarity */
WITH cust_table AS(
SELECT customer_unique_id, AVG(ocost) as avg_order_value, 
        COUNT(*) as order_count, SUM(ocost) as realized_cost,
...
GROUP BY customer_unique_id
    ...
    --(order_count*1.0)/active_period as purch_freq,
